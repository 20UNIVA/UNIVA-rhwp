 ...
 *      → 4-2 fix 이후 insert_text 도 op_stash 적재. stash 3 entry.
 *   2. 초기 텍스트 'C' 확인
 *   3. undo 1회 → 'B' (C → B 역적용)
 *   4. undo 2회 → 'A' (B → A 역적용)
 *   5. undo 3회 → '' (A → '' 역적용; insert_text 직전 빈 문단)
 *   6. undo 4회 → 409 NO_UNDO_AVAILABLE (빈 stash)
...
async function main() {
  ...
  // 5. undo 3 — '' (insert_text 직전 빈 문단)
  u = await postUndo(fileId);
  if (u.status !== 200) throw new Error(`undo 3: ${JSON.stringify(u)}`);
  ir = await getIr(fileId);
  const t3 = paraText(ir) ?? '';
  if (t3 !== '') throw new Error(`undo 3 후 text mismatch: '${t3}'`);
