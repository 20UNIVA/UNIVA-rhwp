 ...
 *   3. lineSpacing 만 변경 → para_shape_id 또 변화 (이전 alignment 는 유지된 채 합쳐진 새 shape)
 *
 * ir-slice raw 모드는 alignment/lineSpacing 을 직접 노출하지 않으므로,
 * 변경 적용은 para_shape_id 변화로 간접 확인한다.
 * 정확한 필드 유지 검사는 다음 단계 (직접 IR fetch 또는 별도 endpoint) 에서 강화.
 *
 * 주의: PartialParagraphStyle 은 #[serde(rename_all = "camelCase")] 라
 * 클라이언트는 `lineSpacing` (snake_case `line_spacing` 아님) 으로 보낸다.
 */
...
async function main() {
  ...
  // 3. lineSpacing 만 변경 (camelCase — PartialParagraphStyle 직렬화 규약)
  const r2 = await postWorkbench(fileId, 'set_paragraph_style', {
    section: 0,
    para: 0,
    style: { lineSpacing: 200.0 },
  });
  if (r2.status !== 200) throw new Error(`lineSpacing 변경 실패: ${JSON.stringify(r2)}`);

  const slice2 = await getIrSlice(fileId, 0, 0, 1, 'raw');
  const shape2 = slice2.paragraphs[0].para_shape_id;
  if (shape2 === shape1) {
    throw new Error(`lineSpacing 변경 후 para_shape_id 동일: shape1=${shape1}`);
  }
