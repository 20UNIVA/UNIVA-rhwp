...
| `char_shape.spacing` | 자간(P): | Character spacing(P): | 字間(P): | 라벨 — 명확화 |
...
| `char_shape.underline.location` | 위치(L): | Position(L): | 位置(L): | 라벨 — *Position* 자리 통일 (다른 자리도 모두 Position) |
...
| `table.placement.keep_with_anchor` | 개체와 조판부호를 항상 같은 쪽에 놓기 | Keep object and anchor on same page | 組版符号と同じページに置く | 토글 — 일본어 *組版符号* (한컴 자리) |
...
| `table.text_dir.rotate_text` | 문자 눕힘(Q) | Lay text(Q) | 文字を寝かせる(Q) | 옵션 — 한컴 자리 *문자 눕힘* |
| `table.text_dir.upright_text` | 문자 세움(U) | Upright text(U) | 文字を立てる(U) | 옵션 — 한컴 자리 *문자 세움* |
...
| `table.border.backslash` | 역슬래시 | Backslash | バックスラッシュ | 옵션 — 일본어 *バックスラッシュ* (`円記号` = ¥ 자리 자체 아님) |
...
| `shape.effects.soft_edge` | 옅은 테두리 | Soft edge | ぼかし | 효과 (한컴 자리 — *옅은*) |
| `shape.effects.soft_edge_none` | 옅은 테두리 없음 | No soft edge | ぼかしなし | 효과 |
| `shape.effects.soft_edge_effect` | 옅은 테두리 효과 | Soft edge effect | ぼかし効果 | 효과 |
...
| `shape.line.arrow_tail` | 꼬리 화살표 | Tail arrow | 尾矢印 | 옵션 |
| `shape.line.arrow_open` | 열린 화살표 | Open arrow | 開放矢印 | 옵션 |
| `shape.line.arrow_solid` | 화살표 | Arrow | 矢印 | 옵션 (기본 화살표) |
| `shape.line.thickness_inside` | 선 굵기 내부 적용(K) | Apply thickness inside(K) | 線の太さを内側に適用(K) | 토글 |
| `shape.line.cap_round` | 둥근 | Round | 丸 | 옵션 (선 끝 모양) |
| `shape.line.cap_flat` | 평면 | Flat | 平面 | 옵션 |
...
| `shape.textbox.fit_text_with_box` | 글에 어울리게 | Fit to text | 文字に合わせる | shape textbox 자리 — 한컴 *글에 어울리게* |
| `shape.textbox.vertical_writing` | 세로쓰기(E): | Vertical writing(E): | 縦書き(E): | 라벨 |
| `shape.textbox.text_horizontal` | 영문 눕힘(O) | Rotate Latin chars(O) | 英文を寝かせる(O) | 옵션 — MS Word 자리 *Rotate Latin* |
| `shape.textbox.text_vertical` | 영문 세움(U) | Upright Latin(U) | 英文を立てる(U) | 옵션 |
...
| `menu.insert.caption_top_center` | 위 가운데 | Top center | 上中央 | 캡션 자리 — 한컴 자리 *위 가운데* |
| `menu.insert.caption_top_right` | 오른쪽 위 | Top right | 右上 | 캡션 자리 |
| `menu.insert.caption_bottom_left` | 왼쪽 아래 | Bottom left | 左下 | 캡션 자리 |
| `menu.insert.caption_bottom_center` | 아래 가운데 | Bottom center | 下中央 | 캡션 자리 — 한컴 자리 *아래 가운데* |
...
| `font.preset.batang` | 바탕 | Batang | Batang | 글꼴 이름 — *고유 명사*. 일본어판도 원어 그대로 (한컴 일본어판 정합) |
| `font.preset.dotum` | 돋움 | Dotum | Dotum | 글꼴 이름 |
| `font.preset.gungseo` | 궁서 | Gungseo | Gungseo | 글꼴 이름 |
| `font.preset.malgun_gothic` | 맑은 고딕 | Malgun Gothic | Malgun Gothic | 글꼴 이름 |
| `font.preset.nanum_gothic` | 나눔고딕 | Nanum Gothic | Nanum Gothic | 글꼴 이름 |
...
| `bookmark.sort.kind` | 종류 | Type | 種類 | 옵션 — *Type* 자리 통일 |
...
| `find.find_replace_title` | 찾아 바꾸기 | Find & Replace | 検索と置換 | dialog 타이틀 — MS Word 자리 *検索と置換* |
...
| 키 | 한국어 | English | 日本語 | 맥락 |
|---|---|---|---|---|
| `charmap.dialog_title` | 문자표 입력 | Symbol | 文字一覧 | dialog 타이틀 — MS Word 자리 *Symbol* 단독 |
...
| `charmap.area.dingbats` | 딩뱃 기호 | Dingbats | ディングバット記号 | 영역 — 일본어 오타 정정 |
...
| `page.section.obj_picture` | 그림(P): | Picture(P): | 画像(P): | 라벨 |
| `page.section.obj_equation` | 수식(E): | Equation(E): | 数式(E): | 라벨 |
| `page.section.obj_table` | 표(A): | Table(A): | 表(A): | 라벨 |
| `page.section.obj_other` | 기타 | Other | その他 | 옵션 |
...
| 키 | 한국어 | English | 日本語 | 맥락 |
|---|---|---|---|---|
| `prefs.dialog_title` | 환경 설정 | Options | 環境設定 | dialog 타이틀 — 한컴 영문판 자리 *Options* |
...
| 키 | 한국어 | English | 日本語 | 맥락 |
|---|---|---|---|---|
| `error.client.init_failed` | 초기화 오류: {message} | Initialization error: {message} | 初期化エラー: {message} | 시작 자리 — placeholder `{message}` 통일 |
