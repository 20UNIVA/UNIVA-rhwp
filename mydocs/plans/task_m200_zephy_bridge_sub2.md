...
| # | 컴포넌트 | 위치 | 책임 |
|---|---|---|---|
| 1 | rhwp 본체 native 신설 | `src/document_core/commands/text_editing.rs` 등 | `replace_runs_native`, `replace_cell_runs_native`, `delete_range_in_cell_multipara_native` 3 신설. *부분 업데이트* 동작 확인 (`apply_para_format_native` 등) + 읽기 메서드 정합 (`get_para_shape_native` 등 — 없으면 신설). |
...
처리는 **옵션 b — 호출 직전 서버가 현재 값 read → Partial 병합 → 완전 객체로 native 호출**. rhwp 본체 native 시그니처는 *변경하지 않는다*. 서버가 클라 `ApplyCharFormatCommand` 패턴 ([command.ts:475-514](../../rhwp-studio/src/engine/command.ts#L475-L514)) 을 답습 — *wasm 호출 직전 `getCharPropertiesAt` 으로 prev 캡처* → 그 자리에 *현재 값을 native read 로 가져옴*.

예시 — `set_paragraph_style { section: 0, para: 3, style: {align: "right"} }` 처리:

```rust
// 1. 현재 값 read
let current = core.get_para_shape_native(0, 3)?;   // ParaShape 완전 객체

// 2. 부분 병합
let merged = ParaShape {
    align: payload.style.align.unwrap_or(current.align),     // 변경
    indent: payload.style.indent.unwrap_or(current.indent),  // 유지
    line_height: payload.style.line_height.unwrap_or(current.line_height), // 유지
    ..current
};

// 3. 완전 객체로 native 호출
core.apply_para_format_native(0, 3, &merged)?;
```
...
3. core.import_hwpx_native(&row.before_blob)?     // 통째 교체
4. DELETE FROM op_stash WHERE seq = ?
5. broadcast ServerEvent::SnapshotRestored { seq, snapshot_base64: base64(&row.before_blob) }
6. 응답 {seq_reverted: <removed seq>, applied: "undo"}
```
...
| # | 범위 | 검증 게이트 |
|---|---|---|
| **2a** | rhwp 본체 native 정비 — `replace_runs_native` / `replace_cell_runs_native` / `delete_range_in_cell_multipara_native` 3 신설 + `apply_para_format_native` / `apply_cell_style_native` / `apply_char_format_native` 의 *부분 업데이트* 동작 코드 확인 + 읽기 메서드 (`get_para_shape_native`, `get_cell_para_shape_native`, `get_char_shape_at_native`) 정합 확인. 없으면 신설. | rhwp `cargo test` 신설 3 native round-trip + 부분 업데이트 unit test. |
...
| **2f** | 클라 onServerEvent ops 12 분기 + SnapshotRestored / Complete 핸들러 + 양방향 e2e + 보고서 — `rhwp-studio/src/main.ts` 확장 + `wasm.importHwpx` 통째 교체 처리 + e2e (액션별 12개 + undo + audit + diff + ir-slice) + 회귀 검증 + stage2/report 작성. | 양방향 e2e 모두 통과 + Sub-1 기존 e2e ws-bridge 회귀 0. |
