...
| 조건 | 결과 |
|---|---|
| 1. 수동 시연 시나리오가 모두 통과한다 | ✅ Incognito 탭 시연 통과 확인 (2026-06-06). PWA SW 캐시 주의는 [stage1 시연 가이드](../working/task_m200_zephy_bridge_stage1.md) 참고. |
...
- `rhwp-studio/src/main.ts` — SessionClient에 `onServerEvent` 콜백 주입 + wasm 변경 후 `eventBus.emit('document-changed')` 발행 (Critical fix #2)
...
665e3c16 Task #zephy-bridge: Sub-1 단계별·최종 결과 보고서
c2a67ec1 Task #zephy-bridge: onServerEvent 에 document-changed emit 추가 (Critical fix #2)
```

Plan/spec/시각화 초안 commit(`8764ff9a`)과 합쳐 *총 13 commit*. 모두 브랜치 `local/task_m200_zephy_bridge`.

## 수동 시연 안내 (사용자 검증 단계)

> **반드시 새 시크릿/Incognito 탭으로 열 것.** `rhwp-studio` 는 `vite-plugin-pwa` 가 자동 생성한 `sw.js` 를 등록하므로, 이미 열어둔 탭은 새 `dist/index-<hash>.js` 가 떴어도 옛 main 번들을 캐시에서 돌려준다. 본 시연에서 실제로 hard reload 만으로는 갱신되지 않는 사례 발생.
...
2. 새 노트북 실행:
   ...
   - 출력된 URL(`http://127.0.0.1:7710/?fileId=...`)을 *시크릿 탭*에 입력.
...
4. (선택) 일반 탭에서 시연하고 싶다면 DevTools → Application → Service Workers → Unregister → Storage → Clear site data 후 hard reload.
...
Sub-1 *접합 인프라* 완성. 자동 검증으로 양방향 WS 채널·서버 SoT·기존 endpoint 회귀 0 확인. 수동 시연도 2026-06-06 사용자 Incognito 탭으로 통과 확인. 수동 시연 중 발견한 Critical fix #2 (onServerEvent → `document-changed` emit 누락) 보정 commit `c2a67ec1` 포함. Sub-2 brainstorm 시점에 위 미해결 4건을 우선 다룬다.
