...
- **브라우저 E2E** (Docker WASM 빌드 후, puppeteer headless): WASM `pkg/` 생성 → `tsc` 에러 0 → `vite build` 성공 → studio에서 키보드 입력이 **디바운스 배치로 서버 IR에 미러링**되고 DELETE 후에도 유지됨을 실측 **PASS** (`rhwp-studio/e2e/ssr-session.test.mjs`)
