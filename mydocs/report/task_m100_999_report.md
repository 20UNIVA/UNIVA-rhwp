...
| **닫았다 다시 열면 화면 복원** | 부팅 시 fileId로 `GET /export` → 서버 상태를 화면에 로드(`restoreSsrSessionIfNeeded`). E2E 실증 |
...
- **브라우저 E2E** (Docker WASM 빌드 후, puppeteer headless): WASM `pkg/` 생성 → `tsc` 에러 0 → `vite build` 성공 → studio에서 키보드 입력이 **디바운스 배치로 서버 IR에 미러링**되고 DELETE 후에도 유지, **새 탭 재진입 시 파일 없이 서버 상태가 화면에 복원**(이전 편집 포함)됨을 실측 **PASS** (`rhwp-studio/e2e/ssr-session.test.mjs`, 7/7)

## 7. 한계 / 후속 과제

- 브라우저 E2E는 Node 23(nvm) + 외부 기동 서버로 수행. CI 통합 시 Node 버전(vite는 20.19+ 요구)·서버 spawn(`E2E_SPAWN_SERVER=1`) 정비 필요.
...
신규: src/model/ir_view.rs
      ...
      rhwp-studio/e2e/ssr-session.test.mjs
